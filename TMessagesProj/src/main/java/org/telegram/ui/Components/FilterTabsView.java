...
public class FilterTabsView extends FrameLayout {
    ...
    public class Tab {
        ...
        public Tab(int i, String t, String e) {
            id = i;
            title = YukiConfig.tabMode != YukiConfig.TAB_TYPE_ICON ? t:"";
            realTitle = t;
            emoticon = e;
        }
        ...
        public boolean setTitle(String newTitle) {
            ...
            title = YukiConfig.tabMode != YukiConfig.TAB_TYPE_ICON ? newTitle:"";
            realTitle = newTitle;
            return true;
        }
    }
    ...
    public class TabView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (animateCounterEnter || counterText != null || showRemove && (isEditing || editingStartAnimationProgress != 0)) {
                ...
                int textSpace = YukiConfig.tabMode != YukiConfig.TAB_TYPE_ICON ? AndroidUtilities.dp(6) : 0;
    ...
    public FilterTabsView(Context context) {
        ...
        listView.setOnItemClickListener((view, position, x, y) -> {
            ...
            if (isEditing) {
                if (position != 0 || YukiConfig.hideAllTab) {
                    int side = AndroidUtilities.dp(6);
                    if (tabView.rect.left - side < x && tabView.rect.right + side > x) {
    ...
    public void checkTabsCounter() {
        ...
        for (int a = 0, N = tabs.size(); a < N; a++) {
            ...
            if (oldWidth != width || invalidated) {
                ...
                requestLayout();
                allTabsWidth = 0;
                if (!YukiConfig.hideAllTab) findDefaultTab().setTitle(LocaleController.getString("FilterAllChats", R.string.FilterAllChats));
                for (int b = 0; b < N; b++) {
...
//                notifyItemMoved(i, i + 1);
    ...
    public class TouchHelperCallback extends ItemTouchHelper.Callback {
        ...
        public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
            if (!YukiConfig.hideAllTab && MessagesController.getInstance(UserConfig.selectedAccount).premiumLocked && (!isEditing || (viewHolder.getAdapterPosition() == 0 && tabs.get(0).isDefault && !UserConfig.getInstance(UserConfig.selectedAccount).isPremium()))) {
        ...
        public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder source, RecyclerView.ViewHolder target) {
            if (!YukiConfig.hideAllTab && MessagesController.getInstance(UserConfig.selectedAccount).premiumLocked && ((source.getAdapterPosition() == 0 || target.getAdapterPosition() == 0) && !UserConfig.getInstance(UserConfig.selectedAccount).isPremium())) {
                return false;
            }
